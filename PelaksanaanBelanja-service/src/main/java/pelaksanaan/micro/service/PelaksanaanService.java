...
public class PelaksanaanService {
  ...
  public void update(Pelaksanaan pelaksanaan) {
    String query = "update Pelaksanaan set tahun_anggaran= :ta, status=:status where id_pelaksanaan= :id";
    Session session = HibernateUtil.getSessionFactory().openSession();
    session.beginTransaction();
    ...
    int result = session.createQuery(
        query).setParameter("ta", pelaksanaan.getTa())
            .setParameter("status", pelaksanaan.getStatus())
